...
struct Xcurl::cache {
  ...
  cache()
      : curl_(curl_easy_init()), res_(CURLE_FAILED_INIT),
...
void Xcurl::init() noexcept {
  if (cache_ && cache_->curl_) {
    cache_->res_ = curl_easy_setopt(cache_->curl_, CURLOPT_URL, url_.c_str());
  }
}

Xcurl::Xcurl(const std::string &url) noexcept
    : url_(url), read_buffer_{}, cache_(std::make_unique<cache>()) {
  init();
}

Xcurl::~Xcurl() = default;

std::string_view Xcurl::operator()() noexcept { return read_buffer(); }
...
bool Xcurl::fetch_data() noexcept {
  ...
  cache_->res_ = ::call_curl(curl_easy_setopt, cache_->curl_, CURLOPT_WRITEDATA,
                             &read_buffer_);
  if (cache_->res_ != CURLE_OK)
    return false;
  ...
  cache_->res_ = ::call_curl(curl_easy_getinfo, cache_->curl_,
                             CURLINFO_RESPONSE_CODE, &cache_->http_code_);
  if (cache_->res_ != CURLE_OK)
    return false;
...
std::string_view Xcurl::read_buffer() noexcept { return read_buffer_; }

CURLcode Xcurl::status() const noexcept { return cache_->res_; }
